...
# 是否开启push 一对一准确推送
PUSH_PLUS_BY_TOKEN = os.getenv("PUSH_PLUS_BY_TOKEN")
...
def sendForJs(title: str, content: str) -> None:
    if not content:
    ...
    content += "\n\n" + text
    t = "require('{}').sendNotify('{}','{}')".format('/ql/data/scripts/sendNotify', title, content)
    cmd = f'node -e "{t}"'
